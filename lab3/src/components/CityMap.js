...
const CityMap = ({ buildings, onUpgrade, cityResources }) => {
  ...
  const calculateTotalSatisfaction = () => {
    if (buildings.length === 0) return 50; // Базова задоволеність

    let totalSatisfaction = 50; // Початкова задоволеність
  ...
  const getSortedAndFilteredBuildings = () => {
    ...
    if (filterCategory !== 'all') {
      filteredBuildings = buildings.filter(b => getBuildingCategory(b) === filterCategory);
    }
  ...
  const getCityStats = () => {
    const stats = {
      ...
      totalValue: buildings.reduce((sum, b) => sum + b.cost, 0),
      averageLevel: buildings.reduce((sum, b) => sum + b.level, 0) / buildings.length || 0,
      maxLevelBuildings: buildings.filter(b => b.level === 2).length
    };
    return stats;
  };
